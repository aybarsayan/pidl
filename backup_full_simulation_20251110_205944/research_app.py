...
def phase_tasks():
    ...
    if substep == 'pre_test':
        ...
        if task_number == 1:
            ...
            st.info("✨ İlk görevinizi tamamladınız! Görev zorluğunu değerlendirin:")

            # Sadece zorluk seviyesi
            task_difficulty_rating = st.select_slider(
                "Bu görevin zorluk seviyesi sizin için nasıldı?",
        ...
        # GÖREV 2+: Tam karşılaştırma (artık her iki AI'yi de deneyimledi)
        else:
            st.markdown("### 🔄 Kısa Karşılaştırma ve Yansıma")
            st.info(f"Bu görevde **{used_ai} AI** kullandınız. Her iki AI tipini de deneyimlediniz, şimdi karşılaştıralım:")

            # Soru 1: Diğer AI daha uygun olur muydu?
            comparison_suitability = st.radio(
                f"Bu görev için **{other_ai} AI** daha uygun olur muydu?",
                ["Hayır, bu görev için kullandığım AI daha uygundu",
                 f"Evet, {other_ai} AI daha uygun olurdu",
                 "Emin değilim / Her ikisi de eşit"],
            ...
            comparison_reason = st.text_area(
                ...
                placeholder="Örnek: Görev çok temeldi, Similar AI yeterli oldu. Complementary gereksiz karmaşık olurdu...",
                height=80,
        ...
        button_text = "✅ Görev Değerlendirmesini Tamamla" if task_number == 1 else "✅ AI Değerlendirmesi ve Karşılaştırmayı Tamamla"
